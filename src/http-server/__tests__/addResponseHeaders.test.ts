...
Deno.test('addResponseHeaders', async t => {
  await step(t, 'should add response headers', async (t) => {
    ...
    await step(t, 'should not add headers that are provided', () => {
      const mockCtx = { responseHeaders: new Headers() };
      const mockNext = fn(() => {});
      addResponseHeaders({ a: 'hehe', b: 'woahdude'})(mockCtx as any, mockNext as any)
    ...
    await step(t, 'should not keep existing headers and overwrite if there are conflicts', () => {
      const mockCtx = { responseHeaders: new Headers({ a: 'bro', c: 'cool' }) };
      const mockNext = fn(() => {});
      addResponseHeaders({ a: 'hehe', b: 'woahdude'})(mockCtx as any, mockNext as any)
