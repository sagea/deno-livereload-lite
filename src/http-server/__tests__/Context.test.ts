...
Deno.test('Context', async t => {
  ...
  await step(t, 'headers property should return original request headers', () => {
    const instance = new Context({
      request: {
  ...
  await step(t, 'respondWith method', async t => {
    await step(t, 'should call sync responseHook and send what it returns', async t => {
      ...
      expect(requestEvent.respondWith).toHaveBeenCalledWith(mockReturnResponse);
      expect(requestEvent.respondWith).toHaveBeenCalledTimes(1);

      await step(t, 'response hook should be passed in response and context', () => {
        expect(responseHook).toHaveBeenCalledWith(mockGivenResponse, ctx);
        expect(responseHook).toHaveBeenCalledTimes(1);
      })
    })

    await step(t, 'should call async responseHook and send what it returns', async t => {
      const mockReturnResponse = new Response('hello');
      const mockGivenResponse = new Response('haha');
      const responseHook = fn<ResponseHook>(async (res) => mockReturnResponse);
      const requestEvent = {
        request: {
          url: 'http://localhost:8080/woahdude/haha.js?woah=true',
          headers: new Headers({ a: 'a' }),
  ...
  await step(t, 'workflow: responseHeaders should add headers to response', async t => {
    const requestEvent = {
      request: {
        url: 'http://localhost:8080/woahdude/haha.js?woah=true',
        headers: new Headers({ a: 'a' }),
    ...
    expect(requestEvent.respondWith).toHaveBeenCalledTimes(1);
    const sentResponse = mock.calls(requestEvent.respondWith)[0].args[0] as Response;
